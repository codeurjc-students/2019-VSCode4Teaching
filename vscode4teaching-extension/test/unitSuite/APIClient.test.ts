...
function createSessionFile(newBaseURL: string, newXsrfToken: string, newJwtToken: string) {
    const v4tPath = path.resolve(client.sessionPath, "..");
    if (!fs.existsSync(v4tPath)) {
        mkdirp.sync(v4tPath);
    }
    fs.writeFileSync(client.sessionPath, newJwtToken + "\n" + newXsrfToken + "\n" + newBaseURL);
}

function expectSessionInvalidated() {
    // Session is invalidated
    expect(mockedCurrentUser.resetUserInfo).toHaveBeenCalledTimes(1);
    expect(fs.existsSync(client.sessionPath)).toBeFalsy();
}
...
describe("API Client", () => {
    ...
    it("should log in", async () => {
        ...
        // Save session file
        expect(fs.existsSync(client.sessionPath)).toBeTruthy();
        const sessionFileText = fs.readFileSync(client.sessionPath).toString();
